	...
	export default {
		...
		methods: {
			...
			to_father(item){
				...
				switch(this.source){
					case "index":
						uni.setStorageSync('manual_community_id',item.id)
						uni.setStorageSync('manual_community_name',item.name)
						uni.setStorageSync('community_id',item.id)
						uni.setStorageSync('exp_manual_community_id_time',(new Date().getTime()/1000)+600)
					break;
					default:
